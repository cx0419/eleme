...
export default {
    ...
    setup() {
        ...
        // 获取默认城市 默认城市为上海 看vuex当中是否为空 如果为空 那就是设置为上海 否则从vuex当中取
        const getGuessCity = async() => {
            //先从localstrage当中去取出来
            useMutation.RECORD_CITY_ID[0](getStore('city_id'));
            useMutation.RECORD_CITY_NAME[0](getStore('city_name'));
            // console.log(store.state.city_name);
            state.guess_city = store.state.city_name===null?'上海':store.state.city_name;
            state.guess_city_id = store.state.city_id===null?'1':store.state.city_id;
        }
        // 获取热门城市
        ...
        const getAllCity = async() => {
            ...
            // console.log(JSON.stringify(state.allCityList))
        }
