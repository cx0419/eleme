...
import {updateAddress,deleteAddress,saveAddress} from '@/api8080/profile'
export default {
    components: {
        HeadTop
    },
    setup() {
        ...
        const addAddress = async() => {
            if(btnStatus) {
                let res = await saveAddress(state.name,state.phone,state.lon,state.lat,state.address,store.state.userInfo.id);
