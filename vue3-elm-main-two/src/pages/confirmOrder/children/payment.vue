...
import { payForOrder } from '@/api8080/order'
...
export default {
    ...
    setup() {
        ...
        // 调起支付请求接口（本项目未接通支付）
        // let payDetail = await payRequest(store.state.orderMessage.id, store.state.userInfo.id)
        ...
        const confrimPay = async () => {  // 确认订单
            state.showAlert = true
            // state.alertText = '当前环境无法支付，请打开官方APP进行付款'
            const resp = await payForOrder(store.state.orderMessage.id)
            console.log(resp)
